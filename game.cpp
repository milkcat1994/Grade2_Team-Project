...
void deleteObject(GameObject* obj)
{
	if (obj) {
		if (object_pool && obj->is_alive)
		{
			...
			if (game_object->bitmap)
			{
				destroyBitmap(game_object->bitmap);
				game_object->bitmap = NULL;
			}
		}
	}
...
//플레이어 제외 Object Pool 비우기
void cleanupPoolExceptPlayer()
{
	int i;
	for (i = 0; i < pool_size; i++) {
		GameObject* game_object = &object_pool[i];
		if (game_object->type == PLAYER || game_object->type == PLAYER_BACK || game_object->type == PLAYER_BULLET) {
			continue;
		}
		else {
			deleteObject(game_object);
